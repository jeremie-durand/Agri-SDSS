...
def generate_processing_report(report_data: dict):
    """
    ...
    for i, data_type in enumerate(sorted(report_data.keys()), start=1):
        ...
        logger.info(f"{i}. {data_type.replace('_', ' ').capitalize()}:")
        for stat_name, value in stats.items():
            logger.info(f"    {stat_name.capitalize():<25}: {value}")
        logger.info("-" * 60)
