...
from gis_pipeline.services.mapping import DefaultMetadata
...
def test_parse_xml_metadata_permission_error_returns_defaults(tmp_path):
    ...
    with patch(
        "gis_pipeline.modules.processing.processing_stac.ET.parse",
...
def test_create_stac_item_from_raster_valid_input(tmp_path):
    ...
    with patch(
        "gis_pipeline.modules.processing.processing_stac._ensure_datetime_with_tz",
...
def test_create_stac_item_from_raster_uses_provided_metadata_and_not_parse_aux(
    ...
    with patch(
        "gis_pipeline.modules.processing.processing_stac._parse_xml_metadata"
    ) as mock_parse:
        item = _create_stac_item_from_raster(
...
def test_build_stac_items_from_cog_success():
    ...
    with patch(
        "gis_pipeline.modules.processing.processing_stac._create_stac_item_from_raster"
    ) as mock_create, patch(
        "gis_pipeline.modules.processing.processing_stac.validate_stac"
    ) as mock_validate:
...
def test_build_stac_items_from_cog_partial_failure_logs_warning(mock_logger):
    ...
    with patch(
        ...
        "gis_pipeline.modules.processing.processing_stac.logger"
    ) as mock_log:
...
def test_stac_api_client_session_configuration(
    ...
    with patch(
        "gis_pipeline.modules.processing.processing_stac.Session"
    ) as mock_session_class:
        with patch(
            "gis_pipeline.modules.processing.processing_stac.Retry"
        ) as mock_retry_class:
            with patch(
                "gis_pipeline.modules.processing.processing_stac.HTTPAdapter"
