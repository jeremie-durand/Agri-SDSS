...
//
// Agri-SDSS override: Vite 8 replaced Rollup with Rolldown as the production
// bundler. Two adjustments vs. upstream were needed to build under Rolldown:
//  - `manualChunks` must be a function; Rollup's object/record form throws
//    "manualChunks is not a function".
//  - `minify: 'esbuild'` requires the `esbuild` package, which Vite 8 no
//    longer bundles (it's now an optional peer dep); Rolldown's own
//    minifier, `'oxc'`, is the new default and needs no extra package.
//    The `esbuild: { drop: [...] }` transform option that went with it has
//    no supported oxc equivalent yet, so it's dropped rather than left dead.
