...
Documentation is linted with [markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2);
rules live in [`.markdownlint-cli2.yaml`](../.markdownlint-cli2.yaml). Install the hooks once with
`pre-commit install` and staged `.md` files are auto-fixed on commit. To check every file at once:

```bash
make lint-md
```

`gis-pipeline/docs/ARGS.md` is excluded because it is generated by `make generate-args` and CI
