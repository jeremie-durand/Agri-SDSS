...
| `chatbot-frontend` | `/chatbot/` | AI assistant UI (Vite/React, served by nginx) |

## Start

```bash
docker compose up -d home stac-browser chatbot-frontend
```

## What it does

- `home` proxies `/stac/` → stac-browser, `/chatbot/` → chatbot-frontend, `/api/` and `/chat/` → chatbot-backend
