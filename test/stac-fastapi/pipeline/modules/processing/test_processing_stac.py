...
def test_create_stac_item_from_vector_valid_input(sample_vector_row):
    """Test _create_stac_item_from_vector with valid input."""
    result = _create_stac_item_from_vector(
        vector_dict=sample_vector_row, unique_id="test_item_1"
    )

    assert isinstance(result, Item)
    assert result.id == "test_item_1"
    assert result.geometry["type"] == "Point"
    assert result.geometry["coordinates"] == (0.0, 0.0)
    assert len(result.bbox) == 4


def test_create_stac_item_from_vector_missing_geometry(sample_vector_row):
    """Test _create_stac_item_from_vector with missing geometry."""
    vector_dict = sample_vector_row.copy()
    vector_dict.pop("geometry")

    with pytest.raises(
        ValueError, match="Vector data does not contain geometry information"
    ):
        _create_stac_item_from_vector(vector_dict=vector_dict, unique_id="test_item")


def test_create_stac_item_from_vector_none_geometry(sample_vector_row):
    """Test _create_stac_item_from_vector with None geometry."""
    vector_dict = sample_vector_row.copy()
    vector_dict["geometry"] = None
...
def test_create_stac_item_from_vector_no_metadata(sample_vector_row):
    """Test _create_stac_item_from_vector with missing metadata."""
    vector_dict = sample_vector_row.copy()
    vector_dict.pop("metadata")

    result = _create_stac_item_from_vector(
        vector_dict=vector_dict, unique_id="test_item"
    )
...
def test_create_stac_item_from_vector_same_start_end_dates():
    """Test _create_stac_item_from_vector with same start and end dates."""
    vec_date = date(2024, 1, 15)  # date-only (start == end)
    ...
    item = _create_stac_item_from_vector(vector_dict=vector_dict, unique_id="vec_1")

    # datetime should be converted to timezone-aware midnight UTC
    expected_dt = datetime(2024, 1, 15, 0, 0, 0, tzinfo=timezone.utc)
    assert item.datetime == expected_dt

    # properties should include auto fields and preserved metadata
    assert item.properties["title"] == "vec_1"
    assert item.properties["data_type"] == "vector"
    assert item.properties["source"] == "vector_processing"
    assert item.properties["prop"] == "val"

    # bbox should match geometry bounds
    expected_bbox = list(gpd.GeoSeries([geometry]).total_bounds)
    assert item.bbox == expected_bbox


def test_create_stac_item_from_vector_with_file_url(sample_vector_row):
    """Test _create_stac_item_from_vector with file URL."""
    vector_dict = sample_vector_row.copy()
    vector_dict["file_url"] = "/path/to/data.shp"
    ...
    assert result.assets["data"].href == "/path/to/data.shp"
    assert result.assets["data"].media_type == "application/octet-stream"


def test_create_stac_item_from_vector_without_file_url(sample_vector_row):
    """Test _create_stac_item_from_vector without file URL."""
    vector_dict = sample_vector_row.copy()
    vector_dict.pop("file_url")
...
def test_create_stac_item_from_vector_polygon_geometry(sample_vector_row):
    """Test _create_stac_item_from_vector with polygon geometry."""
    polygon = Polygon([(0, 0), (1, 0), (1, 1), (0, 1)])
    vector_dict = sample_vector_row.copy()
    vector_dict["geometry"] = polygon
    vector_dict["metadata"] = {"type": "polygon"}

    result = _create_stac_item_from_vector(
        vector_dict=vector_dict, unique_id="test_polygon"
    )
...
def test_create_stac_item_from_vector_point_geometry(sample_vector_row):
    """Test _create_stac_item_from_vector with point geometry."""
    point = Point(10, 20)
    vector_dict = sample_vector_row.copy()
    vector_dict["geometry"] = point
    vector_dict["metadata"] = {"type": "point"}

    result = _create_stac_item_from_vector(
        vector_dict=vector_dict, unique_id="test_point"
    )

    assert result.geometry["type"] == "Point"
    assert result.geometry["coordinates"] == (10.0, 20.0)
    assert len(result.bbox) == 4
    assert result.bbox == [10.0, 20.0, 10.0, 20.0]


def test_create_stac_item_from_vector_line_string_geometry(sample_vector_row):
    """Test _create_stac_item_from_vector with LineString geometry."""
    from shapely.geometry import LineString

    line = LineString([(0, 0), (1, 1), (2, 2)])
    vector_dict = sample_vector_row.copy()
    vector_dict["geometry"] = line
    vector_dict["metadata"] = {"type": "line"}

    result = _create_stac_item_from_vector(
        vector_dict=vector_dict, unique_id="test_line"
    )

    assert result.geometry["type"] == "LineString"
    assert result.geometry["coordinates"] == ((0.0, 0.0), (1.0, 1.0), (2.0, 2.0))
    assert len(result.bbox) == 4
    assert result.bbox == [0.0, 0.0, 2.0, 2.0]


# ------------------------------------------
# Test cases for build_stac_items_from_gdf()
# ------------------------------------------
def test_build_stac_items_from_gdf_valid_input(sample_gdf):
    """Test build_stac_items_from_gdf with valid GeoDataFrame."""
    ...
    with patch(
        ...
        result = build_stac_items_from_gdf(gdf=sample_gdf, source_table_name=table_name)

    assert len(result) == 3
    assert all(isinstance(item, Item) for item in result)

    # Check IDs
    expected_ids = ["test_table_1", "test_table_2", "test_table_3"]
    actual_ids = [item.id for item in result]
    assert actual_ids == expected_ids

    # Check source table property
    for item in result:
        assert item.properties["source_table"] == "test_table"
        assert "geometry_type" in item.properties


def test_build_stac_items_from_gdf_empty_geodataframe():
    """Test build_stac_items_from_gdf with empty GeoDataFrame."""
    empty_gdf = gpd.GeoDataFrame({"geometry": []}, geometry="geometry", crs="EPSG:4326")

    result = build_stac_items_from_gdf(gdf=empty_gdf, source_table_name="empty_table")

    assert result == []


def test_build_stac_items_from_gdf_missing_gid():
    """Test build_stac_items_from_gdf when gid column is missing."""
    gdf_no_gid = gpd.GeoDataFrame(
        {
            "name": ["A", "B"],
    ...
    # Should use index when gid is missing
    expected_ids = [f"{table_name}_{i}" for i in range(len(gdf_no_gid))]
    actual_ids = [item.id for item in result]
    assert actual_ids == expected_ids


def test_build_stac_items_from_gdf_geometry_types():
    """Test build_stac_items_from_gdf with different geometry types."""
    ...
    assert len(result) == 2
    # Should have geometry_type property with both types
    for item in result:
        assert "geometry_type" in item.properties
        assert (
            "Point" in item.properties["geometry_type"]
            or "Polygon" in item.properties["geometry_type"]
        )


def test_build_stac_items_from_gdf_no_geometry_types():
    """Test build_stac_items_from_gdf when no geometry types found."""
    empty_geom_gdf = gpd.GeoDataFrame(
        {"geometry": []}, geometry="geometry", crs="EPSG:4326"
    )

    result = build_stac_items_from_gdf(
        gdf=empty_geom_gdf, source_table_name="empty_geom_table"
    )
