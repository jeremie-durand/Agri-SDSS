...
NGINX_LINT_HOSTS := --add-host stac-api:127.0.0.1 \
	--add-host raster-api:127.0.0.1 \
	--add-host vector-api:127.0.0.1 \
	--add-host process-api:127.0.0.1 \
	--add-host stac-browser:127.0.0.1 \
	--add-host chatbot-frontend:127.0.0.1 \
	--add-host chatbot-backend:127.0.0.1 \
	--add-host agriculture.canada.ca:127.0.0.1

lint-nginx:
	@echo "Validating home nginx config (generated by entrypoint.sh)..."
	docker build -q -t agri-sdss-home-nginx-lint -f frontend/home/Dockerfile.home frontend/home
	docker run --rm $(NGINX_LINT_HOSTS) agri-sdss-home-nginx-lint validate
	@echo "Validating chatbot-frontend nginx config..."
	docker run --rm $(NGINX_LINT_HOSTS) \
		-v $(CURDIR)/chatbot/overrides/frontend/nginx.conf:/etc/nginx/conf.d/default.conf:ro \
